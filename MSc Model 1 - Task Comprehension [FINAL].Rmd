...
# Runs included
MODEL_1 <- glmmTMB(number_of_pulls ~ session_type*OT + rank_grouping*session_type + 
                       training + relative_session_number:session_type + (1|subject),
                ziformula = ~1,
                family = nbinom1, data = model.data)


# No Runs Included
MODEL_1_NO_RUN <- glmmTMB(num_pulls_runs_removed ~ session_type*OT + rank_grouping*session_type  + training + relative_session_number:session_type
                            + (1|subject),
                        ziformula = ~1,
...
```{r}
summary(MODEL_1_NO_RUN)

Anova(MODEL_1_NO_RUN, type = 3)

tab_model(MODEL_1_NO_RUN,
          show.est = TRUE, show.ci = 0.95,
          show.p = TRUE, p.style = "numeric",
          show.se = TRUE, show.r2 = TRUE,
          show.zeroinf = FALSE,
...
emm <- emmeans(MODEL_1, ~ rank_grouping | session_type)
contrast(emm, method = "revpairwise", by = "rank_grouping", adjust = "bonferroni")
```

```{r Repeat Probing with Runs Removed}
emm <- emmeans(MODEL_1_NO_RUN, ~ rank_grouping | session_type)
contrast(emm, method = "revpairwise", by = "rank_grouping", adjust = "bonferroni")
```

```{r Probe Session Type x Session Number interaction}
trends <- emtrends(MODEL_1, ~ session_type, var = "relative_session_number", adjust = "bonferroni")
summary(trends)
pairs(trends)
```

```{r Repeat Probing with Runs Removed}
trends <- emtrends(MODEL_1_NO_RUN, ~ session_type, var = "relative_session_number", adjust = "bonferroni")
summary(trends)
pairs(trends)
```
...
low_ranking_graph <-ggplot(low_df, aes(x = factor(session_type), 
                    ...
                    geom_signif(comparisons = list(c("Experimental", "Inaccessible")), 
                                annotations = "N.S.", y_position = 30, textsize = 5, tip_length = 0) +
