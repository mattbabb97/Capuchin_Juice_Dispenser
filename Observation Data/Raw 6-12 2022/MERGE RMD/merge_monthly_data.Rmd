...
```{r}
df = select(df, ObID, Group:AMPM, TimeInSec, everything(), -Timestamp)
```
...
```{r}
write_csv(global, str_c('MONTHLY/WhatsOb_Global_Data_', dir, '.csv'), na = '')
write_csv(adlib, str_c('MONTHLY/WhatsOb_AdLib_Data_', dir, '.csv'), na = '')
```
